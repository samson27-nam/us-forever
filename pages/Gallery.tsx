...
const Gallery: React.FC = () => {
  ...
  const fetchPhotos = async () => {
    try {
      ...
      setPhotos([
        { _id: '1', url: 'https://images.unsplash.com/photo-1518199266791-5375a83190b7', caption: 'First Date', uploadedBy: 'me' }
      ]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12">
      <div className="flex justify-between items-end mb-12">
        <div>
          <h2 className="text-5xl font-serif-elegant font-bold text-stone-900 mb-2">Our Gallery</h2>
          <p className="text-stone-400 font-medium">A collection of every smile and every sunset.</p>
        </div>
        <button 
      ...
      {loading ? (
        <div className="flex flex-col items-center py-40 gap-4">
          ...
          <p className="font-romantic text-3xl text-stone-300">Uncovering moments...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {photos.map((photo) => (
            ...
            >
              <img src={photo.url} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt={photo.caption} />
              <div className="absolute inset-0 bg-stone-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-6">
                <div className="flex justify-end gap-2">
                  <button onClick={() => setSelectedImage(photo)} className="p-3 bg-white/20 backdrop-blur-md rounded-full text-white hover:bg-white/40"><Maximize2 className="w-5 h-5" /></button>
                  <button onClick={() => deletePhoto(photo._id)} className="p-3 bg-rose-500/80 backdrop-blur-md rounded-full text-white hover:bg-rose-600"><Trash2 className="w-5 h-5" /></button>
                </div>
                <div>
      ...
      {showUpload && (
        <div className="fixed inset-0 z-[100] bg-stone-900/40 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in">
          <div className="bg-white w-full max-w-xl rounded-[3rem] p-12 shadow-2xl relative">
            <button onClick={() => setShowUpload(false)} className="absolute top-8 right-8 text-stone-400 hover:text-stone-900"><X /></button>
            <h3 className="text-3xl font-serif-elegant font-bold mb-8">New Memory</h3>
            <form onSubmit={handleUpload} className="space-y-6">
              <div className="aspect-video bg-rose-50 rounded-3xl border-2 border-dashed border-rose-200 flex flex-col items-center justify-center cursor-pointer hover:bg-rose-100 transition-colors relative overflow-hidden">
                {file ? (
                  ...
                  <>
                    ...
                    <p className="text-sm font-bold text-rose-400">Click to pick a photo</p>
                  </>
                )}
              ...
              >
                {uploading ? <Loader2 className="animate-spin" /> : 'Save Moment ❤️'}
              </button>
            </form>
          </div>
        </div>
      ...
      {/* Preview Modal */}
      {selectedImage && (
        <div className="fixed inset-0 z-[110] bg-stone-900/95 flex items-center justify-center p-10 animate-in fade-in zoom-in" onClick={() => setSelectedImage(null)}>
          <img src={selectedImage.url} className="max-w-full max-h-full object-contain rounded-xl" />
          <p className="absolute bottom-10 text-white text-2xl font-romantic">{selectedImage.caption}</p>
        </div>
      )}
    </div>
  );
