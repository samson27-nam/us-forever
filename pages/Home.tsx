    ...
    <div className="max-w-6xl mx-auto px-6 py-20 flex flex-col items-center text-center">
      <div className="space-y-8 animate-in fade-in slide-in-from-bottom-10 duration-1000">
        ...
        <p className="text-2xl md:text-3xl text-stone-400 font-light italic max-w-2xl mx-auto leading-relaxed">
          "Every picture tells a story of a heart that found its home. Let's keep writing ours, one memory at a time."
        </p>
        ...
        <div className="pt-10">
          ...
          >
            Explore Our Gallery <ChevronRight className="w-6 h-6 group-hover:translate-x-1 transition-transform" />
          </Link>
        </div>
      </div>
      ...
      <div className="mt-32 w-full grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          ...
          <div key={i} className="aspect-[3/4] rounded-3xl overflow-hidden shadow-xl border-4 border-white rotate-2 hover:rotate-0 transition-all duration-500">
            <img src={url} className="w-full h-full object-cover" alt="Romantic Preview" />
          </div>
        ))}
      </div>
    </div>
